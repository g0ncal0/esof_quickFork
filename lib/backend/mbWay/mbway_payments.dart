...
Future<Map<bool, String>> payWithMbway(
    String phoneNumber, String paymentAmount) async {
  try {
    ...
    String transactionSignature_dict_response =
        responseData['transactionSignature'];
    Map<String, dynamic> returnStatus_dict_response =
        responseData['returnStatus'];
    ...
    if (response.body.isEmpty) {
      ...
      return {
        false: "MbWay is not responding at the moment, please try again later."
      };
    } else {
      //print(response.body);
      RegExp regExp = RegExp(r'success', caseSensitive: false);
      bool hasMatch =
          regExp.hasMatch(json.decode(response.body)['paymentStatus']);

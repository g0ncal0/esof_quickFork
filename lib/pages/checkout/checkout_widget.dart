...
class CheckoutWidget extends StatefulWidget {
  ...
  String createQrCode() {
    ...
    String qrcode =
        generateRandomId(10); // Generate a random string of length 10
    DateTime now = DateTime.now();
    String timeStamp =
        now.microsecondsSinceEpoch.toString(); // Unique timestamp

    // Concatenate the timestamp with a random string
    String qrCode = qrcode +
        timeStamp +
        generateRandomId(6); // 6 additional random characters
  ...
  void showPaymentStatus(BuildContext context, bool paymentSuccessful) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
              paymentSuccessful ? 'Compra bem-sucedida' : 'Erro no pagamento'),
    ...
    if (mealQuery.docs.isNotEmpty) {
      ...
      if ("Meat " ==
          _model.radioButtonValue
              ?.substring(0, _model.radioButtonValue?.indexOf("-"))) {
        DocumentSnapshot docSnapshot = await docRef.get();
        Map<String, dynamic>? data =
            docSnapshot.data() as Map<String, dynamic>?; // Explicit cast
                                    ...
                                    Expanded(
                                      child: Align(
                                        ...
                                        child: FlutterFlowRadioButton(
                                          ...
                                          radioButtonColor: Colors.deepPurple,
                            ...
                            child: FFButtonWidget(
                              onPressed: () async {
                                ...
                                final paymentSuccessful =
                                    paymentResponse.paymentId != null &&
                                        paymentResponse.paymentId!.isNotEmpty;
                          ...
                          FFButtonWidget(
                            onPressed: () async {
                              ...
                              if (phoneNum.isNotEmpty) {
                                var result = await payWithMbway(phoneNum,
                                    _model.fullMeal ? '2.75' : '2.95');

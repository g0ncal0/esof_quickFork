        ...
        try {
          _model.mealInfoLunch = await queryWeekelyMealsRecordOnce(
            queryBuilder: (weekelyMealsRecord) => weekelyMealsRecord.where(
              'weekdayMeal',
              isEqualTo:
                  '${DateFormat('EEEE').format(DateTime.now()).toLowerCase()}-lunch',
            ),
            singleRecord: true,
          ...
          _model.mealInfoDinner = await queryWeekelyMealsRecordOnce(
            queryBuilder: (weekelyMealsRecord) => weekelyMealsRecord.where(
              'weekdayMeal',
              isEqualTo:
                  '${DateFormat('EEEE').format(DateTime.now()).toLowerCase()}-dinner',
            ),
            singleRecord: true,
          ).then((s) => s.firstOrNull);
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if (session != null) {
        ...
        if (image_small == null || image_small!.isEmpty) {
          image_small =
              (await getImage(session.cookies, session.username)).bodyBytes;
        }
        name = jsonDecode(
                (await getName(session.cookies, session.username)).body)['nome']
            .toString()
            .split(' ')[0];
      } else {
              ...
              Padding(
                ...
                child: InkWell(
                    ...
                    child: ClipRRect(
                      ...
                      child: Image.memory(image_small!,
                          fit: BoxFit.cover), // Same radius value as above
                    )),
              )
          ],
          centerTitle: false,
                            ...
                            children: [
                              ...
                              Align(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    () {
                                      _model.descriptionsLunch?.forEach(
                                          (name) => tempLunch += "$name\n");
                                      return tempLunch;
                                    }(),
                            ...
                            children: [
                              ...
                              Align(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    () {
                                      _model.descriptionsDinner?.forEach(
                                          (name) => tempDinner += "$name\n");
                                      return tempDinner;
                                    }(),
                                    textAlign: TextAlign.start,
                                    style: FlutterFlowTheme.of(context)
